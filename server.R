...
shinyServer(function(input, output) {
  ...
  output$RE_dropdown0 <- renderUI({
    selectInput("RE_dropdown0", label = "Grouped Fixed Effects", choices = unname(fixed_effects()), multiple = T)
  })
  ...
  make_model <- reactive({
    ...
    # Specify model, checking for errors
    model <- tryCatch({
      makeLmer(formula, fixef=fixef, VarCorr=rand, sigma=res, data=getDF())
    }, error = function(e){
      if (length(str_split_1(input$FE_levels0, ",")) != length(str_split_1(input$FE_ES0, ",")) + 1) {
        validate(need(FALSE, "Incorrect number of effect sizes for levels of factor" ))
      }
    else{
      validate(need(FALSE, "Model incorrectly specified"))
      }
    })
